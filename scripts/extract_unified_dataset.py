#!/usr/bin/env python3
"""
Extract positives AND easy negatives from full-episode landmark files.
This ensures all training data comes from the same source, eliminating
the data pipeline confound found in the first experiment.

Also splits existing hard negatives by episode for consistent train/val.
"""
import os
import sys
import gc
...
FPS = 25
WINDOW_SIZE = 25
POS_STRIDE = 3   # denser for limited positives
NEG_STRIDE = 5   # sparser for abundant negatives
SEED = 42
EASY_NEG_BUFFER = 5.0  # seconds buffer around annotations
EASY_NEG_MIN_DUR = 2.0  # minimum segment duration in seconds
MIN_WRIST_VEL = 0.005
...
VAL_FRACTION = 0.2  # 80/20 episode split


def extract_windows(seq, window_size=WINDOW_SIZE, stride=POS_STRIDE):
    """Extract sliding windows from a (N, D) feature sequence."""
    N, D = seq.shape
    if N < window_size:
...
def find_easy_negative_intervals(ann_times, episode_duration):
    """Find intervals >5s from any annotation, min 2s duration."""
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("EXTRACT UNIFIED DATASET FROM FULL-EPISODE LANDMARKS")
    print("=" * 70)

    # Load annotations
    auto_ann = pd.read_csv(AUTO_ANN_PATH)
    auto_ann['video_id'] = auto_ann['video_id'].astype(str)
    print(f"Total annotations: {len(auto_ann):,}")

    # Find episodes with full landmarks (excluding held-out)
    ...
    # Filter annotations to episodes with landmarks
    ...
    print(f"Annotations in these episodes: {len(ann_with_landmarks):,}")
    ...
    n_train_ann = len(ann_with_landmarks[ann_with_landmarks['video_id'].isin(train_eps)])
    n_val_ann = len(ann_with_landmarks[ann_with_landmarks['video_id'].isin(val_eps)])
    print(f"Annotations: {n_train_ann} train, {n_val_ann} val")
    ...
    for i, ep_id in enumerate(sorted(ep_ids_with_landmarks)):
        ...
        # ── Extract positives ──
        ep_pos_windows = []
        for _, row in ep_ann.iterrows():
            f_start = int(row['start'] * FPS)
            f_end = min(int(row['end'] * FPS), n_frames)
            if f_end - f_start < WINDOW_SIZE:
                stats['n_pos_ann_skipped_short'] += 1
                continue

            seg_lm = landmarks[f_start:f_end]
            feats = extract_all_frame_features(seg_lm)
            windows = extract_windows(feats, WINDOW_SIZE, POS_STRIDE)
            if len(windows) > 0:
                ep_pos_windows.append(windows)
            stats['n_pos_ann_processed'] += 1

        if ep_pos_windows:
            ep_pos = np.concatenate(ep_pos_windows)
            if is_train:
                pos_windows_train.append(ep_pos)
            else:
                pos_windows_val.append(ep_pos)

        # ── Extract easy negatives ──
        ...
        if (i + 1) % 25 == 0 or i == 0:
            n_pos_t = sum(len(w) for w in pos_windows_train)
            n_pos_v = sum(len(w) for w in pos_windows_val)
            n_neg_t = sum(len(w) for w in easy_neg_train)
            n_neg_v = sum(len(w) for w in easy_neg_val)
            print(f"  [{i+1}/{len(ep_ids_with_landmarks)}] "
                  f"pos: {n_pos_t+n_pos_v:,} (t:{n_pos_t:,} v:{n_pos_v:,}), "
                  f"easy_neg: {n_neg_t+n_neg_v:,} (t:{n_neg_t:,} v:{n_neg_v:,})")

    # ── Stack and save positives ──
    print("\nSaving positives...")
    if pos_windows_train:
        pt = np.concatenate(pos_windows_train)
        np.savez_compressed(os.path.join(OUT_DIR, 'positives_train.npz'), windows=pt)
        print(f"  positives_train: {pt.shape}")
        del pt
    if pos_windows_val:
        pv = np.concatenate(pos_windows_val)
        np.savez_compressed(os.path.join(OUT_DIR, 'positives_val.npz'), windows=pv)
        print(f"  positives_val: {pv.shape}")
        del pv
    del pos_windows_train, pos_windows_val
    gc.collect()

    # ── Stack and save easy negatives (subsample to avoid OOM) ──
    # We only need ~2x positives worth of easy negatives, not 2M+ windows
    MAX_EASY_NEG_TRAIN = 250_000  # ~2x positives
    MAX_EASY_NEG_VAL = 60_000

    print("Saving easy negatives (subsampled to avoid OOM)...")
    rng2 = np.random.RandomState(SEED + 1)

    if easy_neg_train:
        # Count total windows across chunks
        ...
        if total_easy_train > MAX_EASY_NEG_TRAIN:
            # Subsample: pick random chunks + trim
    ...
    if os.path.exists(HARD_NEG_META_PATH) and os.path.exists(HARD_NEG_PATH):
        ...
        # Map segments to episodes — metadata has one row per segment
        # Each segment produces multiple windows. We need to figure out
        # which windows belong to which episode.
        # The metadata has columns: episode_id, gap_start, gap_end, n_windows, etc.
        ...
        if 'episode_id' in meta_cols and 'n_windows' in meta_cols:
            ...
            # Build window-to-episode mapping
            ep_ids_per_window = []
    ...
    stats['train_episodes'] = sorted(train_eps)
    stats['val_episodes'] = sorted(val_eps)
    stats['n_train_episodes'] = len(train_eps)
    stats['n_val_episodes'] = len(val_eps)
    ...
    print(f"{'='*70}")
    print(f"  Annotations processed: {stats['n_pos_ann_processed']}")
    print(f"  Annotations skipped (short): {stats['n_pos_ann_skipped_short']}")
    print(f"  Easy negative segments: {stats['n_easy_neg_segments']}")
